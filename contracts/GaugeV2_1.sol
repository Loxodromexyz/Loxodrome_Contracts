...
contract GaugeV21 is ReentrancyGuard, Ownable {
    ...
    function getReward(address _account, address[] memory _tokens) public nonReentrant updateReward(msg.sender) {
        require(msg.sender == DISTRIBUTION);
        uint256 reward = rewards[_account];
        if (reward > 0) {
            rewards[_account] = 0;
            rewardToken.safeTransfer(_account, reward);
            emit Harvest(_account, reward);
        }
